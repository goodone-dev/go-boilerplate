...
func NewServer(ctx context.Context, client rabbitmq.Client, config ServerConfig) *Server {
	...
	if err != nil {
		logger.With().Fatalf(ctx, err, "❌ RabbitMQ failed to declare RPC queue")
		return nil
	}
...
func (s *Server) Serve(ctx context.Context, handler RequestHandler) error {
	deliveryHandler := func(ctx context.Context, delivery amqp.Delivery) error {
		logger.With().Infof(ctx, "📥 RabbitMQ received request from queue %s with correlation ID %s", s.queueName, delivery.CorrelationId)

		// Process the request
		response, err := handler(ctx, delivery.Body, delivery.Headers)
		if err != nil {
			logger.With().Errorf(ctx, err, "❌ RabbitMQ error processing request")
			// Send error response
